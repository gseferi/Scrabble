...
numOcc :: Char -> String -> Int
numOcc c [] = 0
numOcc c (x:xs) = if c == x then (1 + numOcc c xs) else numOcc c xs
...
formable [] ys c = Nothing
formable (x:xs) ys c = submultiset (x:xs) (ys++[c]) 
...
invertBoard xss = [reverse xs | xs <- reverse xss]

testBoard = [[Nothing, Just 'a', Nothing], [Just 'b', Nothing, Nothing], [Nothing, Nothing, Just 'c']]
testBoard' = [[Nothing, Nothing, Nothing], [Nothing, Just 'b', Nothing], [Nothing, Nothing, Nothing]]
...
up b acc = if length ([x | x <- [head xs | xs <- b], x /= Nothing]) > 0 then (7 - acc) 
           else up zs (acc + 1)
           where 
           zs = [tail xs | xs <- b]

down b acc = if length ([x | x <- [head (reverse xs) | xs <- b], x /= Nothing]) > 0 then (7 - acc) 
             else down zs (acc + 1)
             where 
             zs = [reverse (drop 1 (reverse xs)) | xs <- b]

left b acc = if length ([x | x <- (head b), x /= Nothing]) > 0 then (7 - acc)
             else left zs (acc + 1)
             where
             zs = tail b

right b acc = if length ([x | x <- (last b), x /= Nothing]) > 0 then (7 - acc)
              else right zs (acc + 1)
              where
              zs = reverse (drop 1 (reverse b))

upAndDown [] u d = []
upAndDown (b:board) u d = [[Nothing | n <- [1..u]] ++ b ++ [Nothing | n <- [1..d]]] ++ upAndDown board u d 

leftAndRight board l r = [[Nothing | n <- [1..length (head board)]] | m <- [1..l]] ++ board ++ [[Nothing | n <- [1..length (head board)]] | m <-[1..r]]

autoResize b = leftAndRight (upAndDown b (up b 0) (down b 0)) (left b 0) (right b 0)
...
-- that board, put the move on the board an = d return the resulting board. You
-- can assume that the given move is valid on the given board.

writeMove' :: Move -> Board -> Board
writeMove' = Bram.writeMove . read . show

writeMove move@(word, ((x, y), _)) b = writeMove2 word (movePositions move) b

writeMove2 [] [] b = b
writeMove2 _ [] b = undefined
writeMove2 [] _ b = undefined
writeMove2 (l:word) (pos:poss) b = writeMove2 word poss (writeLetter l pos b)

writeLetter c (0, 0) b = [[Just c] ++ tail (head b)] ++ tail b
writeLetter c (0, y) b = [ [(head b) !! i | i <- [0..y-1]] ++ [Just c] ++ [(head b) !! i | i <- [y+1..(length (head b) - 1)]] ] ++ tail b
writeLetter c (x, 0) b = [(b !! i) | i <- [0..x-1]] ++ [ [Just c] ++ tail (b !! x) ] ++ [b !! i | i <-[x+1..(length b - 1)]]
writeLetter c (x, y) b = [ b !! i | i <- [0..(x-1)]] ++ [ [(b !! x !! i) | i <- [0..y-1]] ++ [Just c] ++ [b !! x !! i | i<- [y+1..(length (b !! x) - 1)]] ] ++ [b !! i| i <- [(x+1)..(length b - 1)]]
...
allWords1 xss c = [xs | xs <- xss, elemIndex c xs /= Nothing]
...
isValid xs index m n = if ((index <= m) && (length xs - index - 1 <= n)) then True else False 

allWords2 xss c m n = allWords2a (allWords1 xss c) c m n 

allWords2a [] c m n = []
allWords2a (xs:xss) c m n = [(xs, index) | index <- elemIndices c xs, (isValid xs index m n) == True] ++ (allWords2a xss c m n)
...
allWords3 xss ys c m n = allWords2a [xs | xs <- xss, (formable xs ys c) /= Nothing] c m n
...
greedyBestMove dict rack b =  maximumBy (comparing moveScore) (allMoves dict rack b)
...
aiOneMove dict = do
    ...
    let filledRack = fillRack r ls
    let newLs = removeFromLS r ls    

    let myMove = fullMove dict filledRack b

    let removeIntersection move = if (move /= Nothing) then delete (removeJust (getIntersection move b)) (getWord move) else []

    let r' = if (myMove == Nothing) then take 7 newLs else filledRack \\ (removeIntersection myMove)

    let b2 = if (myMove /= Nothing) then autoResize (writeMove (removeJust myMove) b) else b
    ...
    let ls' = if (myMove == Nothing) then drop 7 newLs else newLs

    let oppMoves' = map (read.show) oppMoves'b
    put (b', r', ls', oppMoves')
    return (read (show myMove))

fillRack rack ls = rack ++ take (7 - length rack) ls   

removeFromLS rack ls = drop (7 - length rack) ls

fullMove dict rack board = greedyBestMove dict rack board

getWord (Just move@(word, _)) = word

getIntersection (Just move) b = head [(b !! x !! y) | (x, y) <- (movePositions move), (b !! x !! y) /= Nothing]
...
--
--   Bram.connectAI b (convertAItoBram (ai sowpods),"abcdefgqqqqqqqhijklmnopqrstuvwxyz") (Bram.ai sowpods, "abcdefghijklmnopqrstuvwxyz")
--
...
ai :: Dict -> AI
ai dict = aiBegin dict

aiBegin dict board ls oppMoves = [move] ++ (aiAfter dict b' ls' oppMoves' r')
                                where (move, (b', r', ls', oppMoves')) = runState (aiOneMove dict) (autoResize board, "", ls, oppMoves)

aiAfter dict board ls oppMoves rack = [move] ++ (aiAfter dict b' ls' oppMoves' r')
                                where (move, (b', r', ls', oppMoves')) = runState (aiOneMove dict) (autoResize board, rack, ls, oppMoves)
...
connectAI :: Board -> (AI, LetterStream) -> (AI, LetterStream) -> [FullMove]
connectAI b (ai1, ls1) (ai2, ls2) = connectAI1 (ai1,ls1,[]) (ai2,ls2,[]) b 1

connectAI1 (ai1,letters1,fullm1) (ai2,letters2,fullm2) b n= [head (reverse move1)] ++ [head (reverse move2)] ++ (connectAI1 (ai1,letters1,move1) (ai2,letters2,move2) b (n+1))
                                       where
                                       move1 = take n (ai1 b letters1 (fullm2 ++ [Nothing]))
                                       b' = if (move == Nothing) then b else writeMove (removeJust move) b 
                                          where move = head move1
                                       move2 = take n (ai2 b' letters2 ((drop 1 move1) ++ [Nothing])) 
